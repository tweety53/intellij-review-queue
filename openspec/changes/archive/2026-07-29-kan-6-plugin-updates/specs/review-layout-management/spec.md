...
**Reason**: KAN-6 asks for the reviewer to be left with the diff alone. Hiding one window of many
left the Terminal, Git, Run, Problems and every third-party window on screen, so the requirement's
central claim — "only the Project tool window" — is the thing being changed, not a detail of it.

**Migration**: Replaced by *A pass hides every visible tool window* below, which keeps all four of
this requirement's scenarios verbatim in meaning and widens the hidden set. No stored state migrates:
the persisted record is a list of ids either way, and a record written by the old behaviour names a
subset of what the new one would have hidden.
